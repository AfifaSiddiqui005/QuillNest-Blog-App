...
import { ArrowLeft, Sparkles, Send, Trash2, Heart } from "lucide-react";
...
export default function PostDetails({ params }) {
	...
	async function fetchLikeData() {
		...
		const userLiked = likes?.find((like) => like.user_id === session.user.id);
	...
	if (loading) {
        ...
        `,
				...
				<div className="relative z-10 flex min-h-screen items-center justify-center">
					<p className="text-zinc-300 text-lg">Loading story...</p>
				</div>
			</main>
		);
	}
    ...
    `,
			...
			<div className="relative z-10 mx-auto max-w-5xl px-4 sm:px-6 py-8 sm:py-14">
				<div className="mx-auto max-w-3xl">
					{/* Back */}
					<Link
						href="/"
						className="
  mb-5 sm:mb-8 inline-flex items-center gap-3
  rounded-2xl
...
">
					...
					{post.image_url && (
						<div className="relative mb-12 overflow-hidden rounded-[36px] border border-white/[0.06] shadow-[0_20px_80px_rgba(0,0,0,0.5)]">
							<img
								src={post.image_url}
								alt={post.title}
								className="h-[240px] sm:h-[420px] w-full object-cover"
							/>
					...
					{/* Author */}
					<div className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
						{/* Author */}
						<button
							onClick={() => router.push(`/writer/${post.users?.id}`)}
							className="
  ...
  ">
							{post.users?.avatar_url ? (
								<img
									...
									alt="Author"
									className="
        h-11 w-11 sm:h-14 sm:w-14 rounded-full object-cover
        transition-all duration-300
      "
								/>
							) : (
  ...
  ">
							...
							<span className="text-sm">
								...
								<span className="text-sm sm:hidden">{likeCount} likes</span>
							</span>
						</button>
					</div>

					{/* Title */}
					<h1 className="mb-8 max-w-4xl text-2xl sm:text-4xl md:text-6xl lg:text-7xl leading-tight tracking-[-0.05em]">
						{post.title}
					</h1>
					...
					{/* Body */}
					<div
						className="mb-20 max-w-3xl whitespace-pre-line text-[15px] sm:text-[18px] md:text-[20px]
leading-8 sm:leading-[2.3]
tracking-[0.01em] text-zinc-300">
						{post.body}
					</div>

					{/* Comments */}
					<div className="rounded-[36px] border border-white/5 bg-white/[0.03] backdrop-blur-[20px] p-4 sm:p-8 shadow-[0_0_80px_rgba(255,255,255,0.02)]">
									...
									<div className="mb-3 flex items-center gap-2 sm:gap-3">
										{comment.users?.avatar_url ? (
											<img
												...
												alt="Reader"
												className="
        h-8 w-8 sm:h-11 sm:w-11 rounded-full object-cover
      "
											/>
										) : (
											<div
