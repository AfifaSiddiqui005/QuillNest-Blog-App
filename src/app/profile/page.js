...
px-5 py-6 sm:px-8 sm:py-10 md:px-12 md:py-14
        ">
				<button
  ...
  ">
				...
				{/* Heading */}
				<h1 className="mb-3 text-2xl sm:text-3xl md:text-4xl font-semibold tracking-[-0.04em] text-zinc-100">
					My Profile
				</h1>
...
"
				...
				{/* Avatar Options */}
				<div className="mb-12 grid grid-cols-2 gap-x-3 gap-y-3 sm:grid-cols-4 sm:gap-x-6 sm:gap-y-5 justify-items-center">
					{avatars.map((avatar) => (
						<button
							key={avatar}
							onClick={() => setSelectedAvatar(avatar)}
							className={`
                    flex h-[62px] w-[62px] sm:h-[96px] sm:w-[96px]
...
backdrop-blur-sm
  ...
  ">
    ...
    "
      ...
      ${role === "author" ? "text-white" : "text-zinc-500 hover:text-zinc-300"}
    `}>
							Writer
						</button>
      ...
      text-sm font-medium
      transition-colors duration-300
      ${role === "viewer" ? "text-white" : "text-zinc-500 hover:text-zinc-300"}
    `}>
							Reader
						</button>
					</div>
...
sm:min-w-[180px]
          ...
          ">
				...
				{message && <p className="mt-5 text-zinc-400">{message}</p>}
			</div>
		</main>
	);
}
